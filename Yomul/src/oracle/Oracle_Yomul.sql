...
AS
SELECT N.NO, M.NICKNAME WRITER, M.PROFILEIMG MIMG, V.NO VNO, V.NAME VNAME, V.IMG VIMG, N.TITLE, N.CATEGORY, N.PRICE, N.HP, N.CONTENT, N.NDATE, N.CHATCHECK, N.HITS, F.FILES , N.MAINFILE
...
select no, writer, vno, vname, title, category, price, hp, content, ndate, chatcheck, hits, files, mainfile
...
-- 찜 목록 가져오기
SELECT FL.PRODUCT_NO, P.TITLE, P.PRICE, M.NICKNAME SELLER_NICKNAME, F.IMG
FROM
(SELECT PRODUCT_NO
...
FROM(SELECT	 ARTICLE_NO || '_' || NO || '_' || FILENAME IMG
							, ARTICLE_NO
						FROM YOMUL_FILES
            WHERE ARTICLE_NO = 'P51'
            ORDER BY NO)
            WHERE ROWNUM = 1) F ON P.NO = F.ARTICLE_NO;
