...
DROP VIEW V_Y_REVIEWS_ARTICLES;
...
-- 후기 뷰 생성(후기 + 업체 정보 + 멤버 정보)
CREATE NOFORCE VIEW V_Y_REVIEWS_ARTICLES
AS
SELECT R.NO, R.VENDOR_NO, V.NAME VENDORNAME, R.MEMBER_NO, M.NICKNAME, R.CONTENT
FROM YOMUL_VENDOR_REVIEWS R, YOMUL_MEMBERS M, YOMUL_VENDORS V
WHERE V.OWNER = M.NO AND V.NO = R.VENDOR_NO;
...
-- 업체 후기 목록 조회
SELECT *
FROM( SELECT ROWNUM AS RNO, NO, VENDOR_NO, VENDORNAME, MEMBER_NO, NICKNAME, CONTENT
        FROM(;
				SELECT TO_NUMBER(SUBSTR(NO, 2)) NO_NUM, NO, VENDOR_NO, VENDORNAME, MEMBER_NO, NICKNAME, CONTENT
				FROM
				V_Y_REVIEWS_ARTICLES 
				WHERE
				LOWER(NO) LIKE ('%' || '' || '%')
				OR LOWER(VENDOR_NO) LIKE ('%' || '' || '%')
				OR LOWER(VENDORNAME) LIKE ('%' || '' || '%')
				OR LOWER(MEMBER_NO) LIKE ('%' || '' || '%')
